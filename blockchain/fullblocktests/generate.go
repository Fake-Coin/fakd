...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: fakutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := fakutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*fakutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, fakutil.NewTx(tx))
	}
...
func additionalCoinbase(amount fakutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee fakutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee fakutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee fakutil.Amount) *wire.MsgTx {
