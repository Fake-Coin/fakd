...
const (
	...
	baseSubsidy = 50 * fakutil.SatoshiPerBitcoin
)
...
func IsCoinBase(tx *fakutil.Tx) bool {
...
func IsFinalizedTransaction(tx *fakutil.Tx, blockHeight int32, blockTime time.Time) bool {
	msgTx := tx.MsgTx()
...
func CheckTransactionSanity(tx *fakutil.Tx) error {
	...
	for _, txOut := range msgTx.TxOut {
		...
		if totalSatoshi > fakutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
// difficulty is in min/max range and that the block hash is less than the
// target difficulty as claimed.
func CheckProofOfWork(block *fakutil.Block, powLimit *big.Int) error {
...
// txscript.
func CountSigOps(tx *fakutil.Tx) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *fakutil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func checkBlockSanity(block *fakutil.Block, powLimit *big.Int, timeSource MedianTimeSource, flags BehaviorFlags) error {
...
// CheckBlockSanity performs some preliminary checks on a block to ensure it is
// sane before continuing with block processing.  These checks are context free.
func CheckBlockSanity(block *fakutil.Block, powLimit *big.Int, timeSource MedianTimeSource) error {
...
func ExtractCoinbaseHeight(coinbaseTx *fakutil.Tx) (int32, error) {
...
func checkSerializedHeight(coinbaseTx *fakutil.Tx, wantHeight int32) error {
...
func (b *BlockChain) checkBIP0030(node *blockNode, block *fakutil.Block, view *UtxoViewpoint) error {
...
func CheckTransactionInputs(tx *fakutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		if originTxSatoshi < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", fakutil.Amount(originTxSatoshi))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxSatoshi > fakutil.MaxSatoshi {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				fakutil.Amount(originTxSatoshi),
				fakutil.MaxSatoshi)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if totalSatoshiIn < lastSatoshiIn ||
			totalSatoshiIn > fakutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
