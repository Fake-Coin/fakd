...
// newAddressPubKey returns a new fakutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) fakutil.Address {
	addr, err := fakutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new fakutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) fakutil.Address {
	addr, err := fakutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new fakutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) fakutil.Address {
	addr, err := fakutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the fakutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the fakutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := fakutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := fakutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := fakutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := fakutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := fakutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*fakutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*fakutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*fakutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*fakutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*fakutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
