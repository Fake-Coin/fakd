...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt fakutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(fakutil.Amount(5 * fakutil.SatoshiPerBitcoin))
	...
	txid = genSpend(fakutil.Amount(500 * fakutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(fakutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*fakutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, fakutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := fakutil.Amount(250 * fakutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = fakutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := fakutil.Amount(50 * fakutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := fakutil.Amount(numMatureOutputs * 50 * fakutil.SatoshiPerBitcoin)
