...
const (
	...
	DefaultMinRelayTxFee = fakutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee fakutil.Amount) int64 {
	...
	if minFee < 0 || minFee > fakutil.MaxSatoshi {
		minFee = fakutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *fakutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee fakutil.Amount) bool {
...
func checkTransactionStandard(tx *fakutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee fakutil.Amount,
...
func GetTxVirtualSize(tx *fakutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
